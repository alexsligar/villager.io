...
module.exports = {
    description: 'Delete tag from an item',
    tags: ['api', 'mod'],
    validate: {
        params: {
    ...
    handler: async function (request, reply) {
        ...
        if (credentials.role === 'user') {
            ...
            if (!check_item_tag) {
                throw Boom.conflict('Item doesn\'t have that tag');
            }
            await this.db.item_tags.delete({ item_id, tag_name });
            return reply({ message: 'Tag Successfully deleted' });
        }
    },
    plugins: {
