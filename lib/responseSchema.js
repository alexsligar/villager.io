...
const user_id = Joi.object({
    user_id: Joi.string().guid().required().example(uuid())
});
const user_ids = Joi.array().items(user_id);
const users = Joi.array().items(public_user).label('PublicUsers');
const private_users = Joi.array().items(user).label('PrivateUsers');

exports.user_ids_response = Joi.object({
    data: user_ids
});
...
const list = Joi.object({
    ...
    owner: Joi.string().guid().required().example(uuid()),
